...
const peopleExtended = [
  ...
  // Отдельные массивы для активных и неактивных
  const activePeople = peopleExtended.filter(person => person.isActive);
  const inactivePeople = peopleExtended.filter(person => !person.isActive);
  
  console.log("Активные люди:", activePeople);
  console.log("Неактивные люди:", inactivePeople);